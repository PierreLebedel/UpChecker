        ...
        <div class="join">
            ...
            @foreach ($elements as $element)
                @if (is_string($element))
                    <button class="join-item btn btn-sm hidden md:flex">...</button>
                @elseif (is_array($element))
                    @foreach ($element as $page => $url)

                        @if($page == ($paginator->currentPage()-3) && $page != 1 )
                            <button class="join-item btn btn-sm btn-disabled hidden md:flex px-0">...</button>
                        ...
                        @elseif($page > ($paginator->currentPage()+3) && $page != $paginator->lastPage() )
                            {{-- do nothing --}}
                        @else
                            <button @class([ 'join-item btn btn-sm hidden md:flex', 'btn-active'=> $page == $paginator->currentPage() ])
